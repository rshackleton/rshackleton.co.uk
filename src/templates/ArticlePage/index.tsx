...
import ArticleFooter from '@components/connected/ArticleFooter';
import ArticleHeader from '@components/presentation/ArticleHeader';
...
import { IArticlePageProps } from './ArticlePage.types';
...
const ArticleTemplate: FC<IArticlePageProps> = ({
  ...
  const ogArticleTags = tags.map(tag => <meta key={tag.codename} property="article:tag" content={tag.name} />);
  ...
  const seo = {
    ...
    extraTags: [
      ...
      <meta key="article:author" property="article:author" content="Richard Shackleton" />,
